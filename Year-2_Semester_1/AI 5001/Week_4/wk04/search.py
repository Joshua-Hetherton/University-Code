"""
Search Algorithms (Chapters 3-4)

This module implements various search algorithms from "Artificial Intelligence: A Modern Approach".
It provides both uninformed and informed search strategies for solving problems represented as
state-space search.

The framework consists of:
1. Problem class - Abstract base class defining search problems
2. Node class - Represents nodes in the search tree
3. Search algorithms - Various strategies for finding solutions

Usage Example:
    # Define a problem by subclassing Problem
    class MyProblem(Problem):
        def actions(self, state): return [...]
        def result(self, state, action): return new_state
        def goal_test(self, state): return state == self.goal

    # Create problem instance and solve
    problem = MyProblem(initial_state, goal_state)
    solution = astar_search(problem)
    if solution:
        path = solution.solution()  # sequence of actions

Uninformed Search Algorithms:
    - breadth_first_tree_search: BFS on trees (complete, optimal for unit costs)
    - depth_first_tree_search: DFS on trees (incomplete, not optimal)
    - breadth_first_graph_search: BFS avoiding cycles (complete, optimal)
    - depth_first_graph_search: DFS avoiding cycles (complete in finite spaces)
    - uniform_cost_search: Dijkstra's algorithm (complete, optimal)
    - iterative_deepening_search: Memory-efficient complete search

Informed Search Algorithms:
    - astar_search: A* with f(n) = g(n) + h(n) (optimal with admissible heuristic)
    - greedy_best_first_graph_search: Greedy search using heuristic only
    - recursive_best_first_search: Memory-limited A* variant

Local Search Algorithms:
    - hill_climbing: Greedy local search (fast, can get stuck in local optima)
    - simulated_annealing: Probabilistic hill climbing with cooling schedule
    - genetic_algorithm: Population-based evolutionary search

Example Problems Included:
    - EightPuzzle: Classic sliding puzzle
    - NQueensProblem: N-Queens placement problem
    - GraphProblem: Generic graph search problems
"""
...
class Problem:
    ...
    - A transition model (result function)
    - A goal test
    - A path cost function (optional, defaults to step cost of 1)
    ...
    Example:
        class SimpleProblem(Problem):
            ...
            def actions(self, state):
                return ['left', 'right'] if state < 10 else []

            def result(self, state, action):
                return state + (1 if action == 'right' else -1)
    """
    ...
    def __init__(self, initial, goal=None):
        ...
        Args:
            ...
            goal: The goal state, or None if goal_test needs to be overridden
        """
    ...
    def actions(self, state):
        ...
        Args:
            state: A state in the problem's state space

        Returns:
            List or iterable of actions. The result would typically be a list,
            but if there are many actions, consider yielding them one at a time
            in an iterator, rather than building them all at once.
    ...
    def result(self, state, action):
        ...
        Args:
            ...
            action: An action from self.actions(state)
    ...
    def path_cost(self, c, state1, action, state2):
        ...
        Args:
            c (float): The cost to reach state1
            state1: The previous state
            action: The action taken from state1
            state2: The resulting state

        Returns:
            float: The cost to reach state2 via the given path
        """
        return c + 1
    ...
    def value(self, state):
        ...
        Args:
            state: A state in the problem space

        Returns:
            float: The value/utility of the state

        Raises:
            NotImplementedError: Must be implemented by subclasses using local search
        """
        raise NotImplementedError
...
def breadth_first_tree_search(problem):
    ...
    This is a tree search, so it doesn't check for repeated states.
    It explores nodes level by level, guaranteeing the shortest path
    in terms of number of steps (optimal for unit step costs).

    Args:
        problem (Problem): A Problem instance defining the search space

    Returns:
        Node or None: A Node containing the goal state and path, or None if no solution

    Time Complexity: O(b^d) where b is branching factor, d is depth of solution
    Space Complexity: O(b^d) for storing the frontier

    Warning:
        Repeats infinitely in case of loops since this is tree search.
        Use breadth_first_graph_search for problems with cycles.
    """
...
def depth_first_tree_search(problem):
    ...
    This is a tree search using a LIFO (Last In, First Out) strategy.
    It goes as deep as possible before backtracking. Not optimal and
    ...
    Returns:
        Node or None: A Node containing a goal state and path, or None if no solution

    Time Complexity: O(b^m) where m is maximum depth of search space
    Space Complexity: O(bm) for storing the frontier

    Warning:
        Repeats infinitely in case of loops. May not find optimal solution.
        Can get stuck in deep/infinite branches.
    """
...
def depth_first_graph_search(problem):
    """
    [Figure 3.7]
    Search the deepest nodes first, avoiding repeated states.

    This is the graph search version of DFS that maintains an explored set
    to avoid cycles. Complete for finite search spaces.
    ...
    Time Complexity: O(b^d) in worst case, better with pruning
    Space Complexity: O(bd) for frontier and explored set

    Note:
        Does not get trapped by loops. If two paths reach a state,
        only the first one is used (may not be optimal).
    """
    frontier = [(Node(problem.initial))]  # Stack
...
def breadth_first_graph_search(problem):
    """[Figure 3.11]
    Search the shallowest nodes first, avoiding repeated states.

    This is the graph search version of BFS. Complete and optimal for
    problems with unit step costs. Avoids infinite loops by tracking
    explored states.
    ...
    Time Complexity: O(b^d) where b is branching factor, d is depth of solution
    Space Complexity: O(b^d) for frontier and explored set

    Properties:
        - Complete: Yes (if finite branching factor)
        - Optimal: Yes (for unit step costs)

    Note: This function can be implemented in a single line as:
...
def best_first_graph_search(problem, f, display=False):
    ...
    A generic best-first search algorithm. The search strategy is determined
    by the evaluation function f(node).

    Args:
        problem (Problem): A Problem instance defining the search space
        f (function): Evaluation function f(node) -> number. Lower values are better.
        display (bool): If True, print search statistics
    ...
    Examples:
        ...
        - A*: f(n) = g(n) + h(n) (path cost + heuristic)
        - Uniform cost: f(n) = g(n) (path cost only)
        - Breadth-first: f(n) = depth(n)

    Time Complexity: O(b^d) in worst case, depends on heuristic quality
    Space Complexity: O(b^d) for frontier and explored set

    Note:
        The line "f = memoize(f, 'f')" means that f values will be cached
        on nodes as they are computed. After search, you can examine the
        f values of the returned path.
    """
    f = memoize(f, "f")
    node = Node(problem.initial)
...
def uniform_cost_search(problem, display=False):
    """[Figure 3.14]
    Search by expanding the node with lowest path cost first.

    This is Dijkstra's algorithm applied to search problems. Always finds
    the optimal solution for problems with non-negative step costs.
    ...
    Returns:
        Node or None: A Node containing the optimal goal state and path, or None

    Time Complexity: O(b^⌈C*/ε⌉) where C* is optimal cost, ε is minimum step cost
    Space Complexity: O(b^⌈C*/ε⌉) for frontier and explored set

    Properties:
        - Complete: Yes (if step costs ≥ ε > 0)
        - Optimal: Yes (for non-negative step costs)
    """
...
def astar_search(problem, h=None, display=False):
    ...
    A* is the most widely known form of best-first search. It combines
    the path cost g(n) and heuristic h(n) to guide the search toward
    the goal efficiently.

    Args:
        problem (Problem): A Problem instance defining the search space
        h (function, optional): Heuristic function h(node) -> number.
                               If None, uses problem.h method. Lower values indicate
    ...
    Properties:
        - Complete: Yes (if finite branching factor and step costs ≥ ε > 0)
        - Optimal: Yes (if heuristic is admissible)

    Heuristic Requirements:
        - Admissible: h(n) ≤ h*(n) where h*(n) is true cost to goal
        - Consistent: h(n) ≤ c(n,a,n') + h(n') for any action a

    Note:
        You need to specify the h function when calling astar_search, or
        implement the h method in your Problem subclass.
    """
...
class EightPuzzle(Problem):
    ...
    The goal is to reach a specified configuration by sliding tiles into the blank space.

    State Representation:
        A state is represented as a tuple of length 9, where the element at
        index i represents the tile number at index i (0 represents the blank square).

        Board layout:     State tuple:
        +---+---+---+     (0, 1, 2,
        | 0 | 1 | 2 |      3, 4, 5,
        +---+---+---+      6, 7, 8)
        | 3 | 4 | 5 |
    ...
    Actions:
        - 'UP': Move blank square up (decrease index by 3)
        - 'DOWN': Move blank square down (increase index by 3)
        - 'LEFT': Move blank square left (decrease index by 1)
        - 'RIGHT': Move blank square right (increase index by 1)

    Example:
        initial = (2, 8, 3, 1, 6, 4, 7, 0, 5)  # 0 is blank
        goal = (1, 2, 3, 4, 5, 6, 7, 8, 0)     # Standard goal
        puzzle = EightPuzzle(initial, goal)
        solution = astar_search(puzzle)

    Heuristics:
        - Default: Number of misplaced tiles (implemented in h method)
        - Manhattan distance: Sum of distances each tile is from goal position
        - Linear conflict: Manhattan distance + additional conflicts
    """
    ...
    def __init__(self, initial, goal=(1, 2, 3, 4, 5, 6, 7, 8, 0)):
        ...
        Args:
            initial (tuple): Initial state as 9-element tuple
            goal (tuple): Goal state as 9-element tuple (default: standard goal)
        """
        super().__init__(initial, goal)
    ...
    def find_blank_square(self, state):
        ...
        Returns:
            int: Index (0-8) of the blank square (represented by 0)
        """
    ...
    def actions(self, state):
        ...
        Returns:
            list: List of valid actions ('UP', 'DOWN', 'LEFT', 'RIGHT')

        Note:
            Actions are filtered based on blank square position to prevent
            moving the blank square outside the 3x3 grid.
        """
    ...
    def goal_test(self, state):
        ...
        Args:
            state (tuple): State to test

        Returns:
            bool: True if state matches the goal configuration
        """
        return state == self.goal
    ...
    def check_solvability(self, state):
        ...
        Args:
            state (tuple): State to check for solvability
        ...
        Note:
            ...
            An inversion is when a larger numbered tile appears before
            a smaller numbered tile when reading left-to-right, top-to-bottom.
        """
        inversion = 0
    ...
    def h(self, node):
        ...
        Args:
            node (Node): Node containing the state to evaluate

        Returns:
            int: Heuristic estimate of cost to reach goal

        Note:
            Default heuristic function used is h(n) = number of misplaced tiles.
            This is admissible but not very informed. Manhattan distance
            heuristic typically performs better.
        """
...
def genetic_search(problem, ngen=1000, pmut=0.1, n=20):
    ...
    This function adapts genetic algorithms to work with Problem instances.
    It generates an initial population from the problem's action space and
    evolves solutions using genetic operators.

    Args:
        problem (Problem): A Problem instance that must have a value method for fitness
        ngen (int): Number of generations to evolve (default: 1000)
        pmut (float): Probability of mutation (0.0 to 1.0, default: 0.1)
        n (int): Population size (default: 20)

    Returns:
        Individual: The best individual found, or population if no threshold met

    Requirements:
        The problem must implement:
        - actions(state): Return available actions
        - result(state, action): Return new state after action
        - value(state): Return fitness/utility value for state

    Note:
        This requires the problem to have states that can be evolved through
        genetic operators. The initial population is created by applying
        random actions to the initial state.
    """
...
def genetic_algorithm(
    ...
    Evolves a population of individuals over multiple generations using
    selection, crossover, and mutation to find optimal solutions.

    Args:
        population (list): Initial population of individuals
        fitness_fn (function): Function that evaluates fitness of individuals
        gene_pool (list): Possible values for genes (default: [0, 1] for binary)
        f_thres (float, optional): Fitness threshold to stop early if reached
        ngen (int): Maximum number of generations (default: 1000)
        pmut (float): Probability of mutation per individual (default: 0.1)

    Returns:
        Individual: Best individual found, either by reaching threshold or
                   after ngen generations

    Time Complexity: O(ngen × population_size × fitness_evaluation)
    Space Complexity: O(population_size × individual_length)

    Algorithm:
        1. Evaluate fitness of current population
        2. Check if any individual meets threshold
        3. Create new generation through selection and reproduction
        4. Apply mutation to offspring
        5. Repeat until threshold met or max generations reached
    """
    for i in range(ngen):
        population = [
