...
This module implements logical reasoning systems for AI, covering both
Propositional Logic and First-Order Logic (FOL). These are fundamental
tools for knowledge representation and automated reasoning in AI systems.

=== WHAT IS LOGIC IN AI? ===
Logic provides a formal framework for representing knowledge and reasoning about it.
- Propositional Logic: Deals with statements that are either true or false
- First-Order Logic: Extends propositional logic with objects, relations, and quantifiers

=== CORE DATA TYPES ===
Four essential classes form the foundation:

    KB            Abstract knowledge base - stores logical facts and rules
    KB_Agent      Agent that uses logical reasoning for decision making
    Expr          A logical expression (imported from utils.py)
                  Examples: P, Q, (P & Q), (P ==> Q)
    substitution  Variable bindings as dictionary {variable: value}
                  Example: {x: 'John', y: 'Mary'} means x=John, y=Mary

=== CREATING LOGICAL EXPRESSIONS ===
Use Expr() or the convenient expr() function (from utils):
    - expr('P & Q')           # Conjunction (AND)
    - expr('P | Q')           # Disjunction (OR)
    - expr('~P')              # Negation (NOT)
    - expr('P ==> Q')         # Implication (IF-THEN)
    - expr('P <=> Q')         # Biconditional (IF-AND-ONLY-IF)

WARNING: String operators have comma precedence - use parentheses carefully!
Good: expr('(P & Q) ==> R')   Bad: expr('P & Q ==> R')

=== INFERENCE ALGORITHMS ===
Propositional Logic:
    pl_true          Evaluate truth of sentence given variable assignments
    tt_entails       Truth table method - checks all possible assignments
    pl_resolution    Resolution theorem proving - efficient for many problems
    dpll_satisfiable DPLL algorithm - systematic search with optimizations
    cdcl_satisfiable Modern SAT solver with conflict-driven clause learning
    WalkSAT          Randomized local search - good for satisfiable formulas

First-Order Logic:
    unify            Unification algorithm - matches logical terms
    fol_fc_ask       Forward chaining - derive new facts from existing ones
    fol_bc_ask       Backward chaining - work backwards from goals

=== UTILITIES ===
    to_cnf           Convert to Conjunctive Normal Form (required for resolution)
    diff, simp       Symbolic differentiation and simplification (bonus feature)

=== EDUCATIONAL NOTES ===
- Start with simple propositional examples before moving to FOL
- Truth tables work for small problems but become exponentially expensive
- Resolution is complete for propositional logic but can be slow
- DPLL and CDCL are the basis for modern industrial SAT solvers
- Unification is the key algorithm that makes FOL inference possible

See logic.ipynb for interactive examples and detailed explanations.
"""
...
class KB:
    ...
    A Knowledge Base (KB) is like a database for logical facts and rules.
    Think of it as an AI's "memory" where it stores what it knows about the world.

    === KEY OPERATIONS ===
    - tell(sentence): Add a new fact or rule to the KB
    - ask(query): Check if something follows from what the KB knows
    - retract(sentence): Remove a fact or rule from the KB

    === WHY ask_generator INSTEAD OF ask? ===
    Different types of logic return different kinds of answers:

    Propositional Logic: ask(P & Q) returns True or False
    - Either the KB entails the query or it doesn't

    First-Order Logic: ask(Brother(x, y)) might return multiple answers:
    - {x: Cain, y: Abel}    # Cain is brother of Abel
    - {x: Abel, y: Cain}    # Abel is brother of Cain
    - {x: George, y: Jeb}   # George is brother of Jeb

    So ask_generator yields these substitutions one at a time, and ask()
    returns just the first one (or False if none exist).

    === IMPLEMENTATION PATTERN ===
    To create a specific KB type:
    1. Subclass this KB class
    2. Implement tell() - how to store new information
    3. Implement ask_generator() - how to answer queries
    4. Implement retract() - how to remove information

    See PropKB and FolKB for concrete examples.
    """
...
class PropKB(KB):
    ...
    Stores logical facts using propositional (boolean) variables.
    Example facts: P, Q, (P & Q), (P ==> Q)

    === HOW IT WORKS ===
    1. Converts all sentences to CNF (Conjunctive Normal Form)
    2. Stores each clause separately in self.clauses list
    3. Uses truth table entailment to answer queries

    === CNF CONVERSION EXAMPLE ===
    Input: P ==> Q
    CNF:   ~P | Q     (stored as a clause)

    === PERFORMANCE WARNING ===
    This implementation is intentionally simple but SLOW:
    - No indexing of clauses by variables
    - Uses exponential truth table algorithm
    - For education only - use more efficient KB for real applications

    === WHEN TO USE ===
    - Learning propositional logic concepts
...
#
# These algorithms check logical entailment by exhaustively testing all
# possible truth assignments. Simple but exponentially expensive.
#
# Key Concepts:
# - Model: An assignment of truth values to propositional variables
# - Entailment: KB |= α means α is true in every model where KB is true
...
def tt_entails(kb, alpha):
    ...
    === ALGORITHM OVERVIEW ===
    1. Find all propositional symbols in KB and query
    2. Generate all possible truth assignments (models)
    3. Check: for every model where KB is true, is alpha also true?
    4. If yes for all models, then KB entails alpha

    === EXAMPLE ===
    KB: P ==> Q, P    Query: Q
    Models to check:
    P=T, Q=T: KB true, Q true ✓
    P=T, Q=F: KB false, skip
    P=F, Q=T: KB true, Q true ✓
    P=F, Q=F: KB true, Q false ✗
    Result: KB entails Q

    === TIME COMPLEXITY ===
    O(2^n) where n = number of unique propositional symbols
    Gets very slow with more than ~20 variables!
...
def pl_true(exp, model={}):
    ...
    === WHAT IS A MODEL? ===
    A model is a dictionary that assigns truth values to propositional symbols.
    Example: {P: True, Q: False} means P is true, Q is false

    === THREE-VALUED LOGIC ===
    This function implements three-valued logic:
    - True: Expression is definitely true in this model
    - False: Expression is definitely false in this model
    - None: Cannot determine (some variables unassigned)

    === EXAMPLES ===
    >>> pl_true(P, {P: True})        # P is true when P=True
    True
    >>> pl_true(P, {P: False})       # P is false when P=False
    False
    >>> pl_true(P, {})               # P is unknown when unassigned
    >>> pl_true(P | Q, {P: True})    # P|Q is true when P=True (regardless of Q)
    True
    >>> pl_true(P & Q, {P: True})    # P&Q is unknown when P=True but Q unassigned

    === OPERATOR HANDLING ===
    - Handles all standard logical operators: ~, &, |, ==>, <==, <=>, ^
    - Uses short-circuit evaluation for efficiency
    - Converts implications to disjunctions internally
...
#
# CNF is a standardized form where formulas are written as AND of ORs:
# Example: (A | ~B | C) & (~A | D) & (B | ~C | ~D)
#
# Why CNF? Many inference algorithms (especially resolution) require CNF input.
#
# Conversion Steps (from Page 253):
# 1. Eliminate implications (==>, <==, <=>)
# 2. Move negation inwards using De Morgan's laws
# 3. Distribute AND over OR
# 4. Flatten nested operators
...
def to_cnf(s):
    ...
    - The whole formula is a conjunction (AND) of clauses
    - Literal = a variable (P) or its negation (~P)

    === CONVERSION ALGORITHM ===
    1. Eliminate implications: P ==> Q becomes ~P | Q
    2. Move NOT inwards: ~(P & Q) becomes ~P | ~Q
    3. Distribute: (P & Q) | R becomes (P | R) & (Q | R)

    === EXAMPLES ===
    Input: P ==> Q           Output: ~P | Q
    Input: ~(P & Q)          Output: ~P | ~Q
    Input: (P & Q) | R       Output: (P | R) & (Q | R)

    === WHY CNF? ===
    - Required for resolution theorem proving
    - Simplifies many logical algorithms
...
def associate(op, args):
    ...
    An operator is associative if: (a op b) op c = a op (b op c)
    Examples: +, *, &, | are associative
             -, /, ==>, <== are NOT associative

    === FLATTENING PROCESS ===
    Nested expressions like ((A & B) & C) become (A & B & C)
    This simplifies the structure while preserving meaning.

    === EXAMPLES ===
    Input: [A&B, C, D&E] with op='&'
    Output: A & B & C & D & E  (all ANDs at same level)

    Input: [A|B, C|(D|E)] with op='|'
    Output: A | B | C | D | E  (all ORs at same level)

    === IDENTITY ELEMENTS ===
    - & (AND): identity is True  (True & X = X)
    - | (OR):  identity is False (False | X = X)
...
#
# Resolution is a powerful inference rule for automated theorem proving.
# Works on clauses in CNF form and is both sound and complete for
# propositional logic.
#
# Key Idea: To prove KB |= α, show that KB ∪ {¬α} is unsatisfiable
# Method: Apply resolution rule until either:
#   1. Derive empty clause ⊥ (contradiction found → KB entails α)
#   2. No new clauses can be derived (KB does not entail α)
#
# Resolution Rule: From clauses (P ∨ A) and (¬P ∨ B), derive (A ∨ B)
...
def pl_resolution(kb, alpha):
    ...
    1. Convert KB and ¬α to CNF clauses
    2. Repeatedly apply resolution rule to clause pairs
    3. If empty clause ⊥ is derived, return True (KB entails α)
    4. If no new clauses can be derived, return False

    === RESOLUTION RULE ===
    From: (P ∨ A₁ ∨ ... ∨ Aₙ) and (¬P ∨ B₁ ∨ ... ∨ Bₘ)
    Infer: (A₁ ∨ ... ∨ Aₙ ∨ B₁ ∨ ... ∨ Bₘ)

    === EXAMPLE ===
    KB: {P ∨ Q, ¬P ∨ R}  Query: Q ∨ R
    1. Add ¬(Q ∨ R) = ¬Q ∧ ¬R as clauses: {¬Q, ¬R}
    2. Clauses: {P ∨ Q, ¬P ∨ R, ¬Q, ¬R}
    3. Resolve P ∨ Q with ¬P ∨ R → Q ∨ R
    4. Resolve Q ∨ R with ¬Q → R
    5. Resolve R with ¬R → ⊥ (empty clause)
    6. Return True (contradiction found)

    === COMPLEXITY ===
    - Time: Exponential in worst case
    - Space: Can generate exponentially many clauses
    - Complete: Will find proof if one exists
...
def zm(symbols, clauses):
    ...
    """
    scores = Counter(
        l for c in min_clauses(clauses) for l in disjuncts(c) if l.op == "~"
    )
    return max(symbols, key=lambda symbol: scores[~symbol]), True
...
#
# DPLL is the foundation of modern SAT solvers. It systematically searches
# for satisfying assignments using smart optimizations to prune the search space.
#
# Key Optimizations:
# 1. Unit Propagation: If clause has only 1 unassigned literal, assign it True
# 2. Pure Literal Elimination: If variable appears only positive (or only negative),
#    assign it to satisfy all clauses containing it
# 3. Branching: Try both True/False for remaining variables
#
# Much more efficient than truth tables for satisfiability testing!
...
def dpll_satisfiable(s, branching_heuristic=no_branching_heuristic):
    ...
    1. Convert sentence to CNF clauses
    2. Apply unit propagation (force assignments from unit clauses)
    3. Apply pure literal elimination (optimize obvious assignments)
    4. If conflicts arise, backtrack
    5. If all clauses satisfied, return satisfying model
    6. If no solution possible, return False

    === OPTIMIZATIONS EXPLAINED ===

    Unit Propagation:
    - Clause (P) forces P = True
    - Clause (~P) forces P = False
    - Propagate consequences through other clauses

    Pure Literal Elimination:
    - If P appears only positively, set P = True (satisfies all clauses with P)
    - If P appears only negatively, set P = False (satisfies all clauses with ~P)

    Branching Heuristic:
    - Choose which variable to try next when no forced moves available
    - Good heuristics can dramatically improve performance

    === EXAMPLE ===
    Input: (P | Q) & (~P | Q) & (~Q)
    1. Unit propagation: ~Q forces Q = False
    2. Simplify: (P | False) & (~P | False) = P & ~P
    3. Unit propagation: P forces P = True, ~P forces P = False
    4. Contradiction detected → UNSATISFIABLE

    === COMPLEXITY ===
    - Worst case: Still exponential O(2^n)
    - Average case: Much better than truth tables due to pruning
    - Basis for industrial SAT solvers (with additional optimizations)
...
def dpll(clauses, symbols, model, branching_heuristic=no_branching_heuristic):
    ...
    return dpll(
        clauses, remove_all(P, symbols), extend(model, P, value), branching_heuristic
    ) or dpll(
        clauses,
        remove_all(P, symbols),
        extend(model, P, not value),
        branching_heuristic,
    )
...
def glucose(conflicts, restarts, queue_lbd, sum_lbd, x=100, k=0.7):
    ...
    return (
        len(queue_lbd) >= x
        and sum(queue_lbd) / len(queue_lbd) * k > sum_lbd / conflicts
    )
...
def unit_propagation(clauses, symbols, model, G, dl):
    def check(c):
        ...
        if w1 in model:
            return c in (
                clauses.get_neg_watched(w1)
                if model[w1]
                else clauses.get_pos_watched(w1)
            )
        w2, _ = inspect_literal(clauses.get_second_watched(c))
        if w2 in model:
            return c in (
                clauses.get_neg_watched(w2)
                if model[w2]
                else clauses.get_pos_watched(w2)
            )
        ...
        for c in filter(check, clauses.get_clauses()):
            ...
            if first_watched is None and clauses.get_first_watched(
                ...
                if clauses.update_second_watched(c, model):
                    ...
                    if (
                        second_watched is None
                    ):  # if it is free, then the clause is a unit clause
    ...
    while True:
        for l in prop_symbols(conflict_clause).intersection(conflict_side):
            ...
            if lbd.count(dl) == 1 and first_uip in prop_symbols(conflict_clause):
                return (
                    0 if len(lbd) == 1 else heapq.nlargest(2, lbd)[-1],
                    conflict_clause,
                    len(set(lbd)),
                )
...
class TwoWLClauseDatabase:
    """
    Data structure for efficient clause management using Two-Watched-Literals (2WL).

    === WHAT IS TWO-WATCHED-LITERALS? ===
    An optimization technique for SAT solvers that dramatically improves performance
    by avoiding unnecessary work during unit propagation.

    === KEY INSIGHT ===
    For clause (A ∨ B ∨ C ∨ D ∨ E):
    - We only "watch" two literals (say A and B)
    - As long as A or B might be true, we ignore this clause
    - Only when both A and B become false do we need to:
      * Find a new literal to watch, OR
      * Detect that clause is unit (only 1 unassigned literal), OR
      * Detect that clause is unsatisfied (conflict)

    === WHY IS THIS EFFICIENT? ===
    Traditional approach: Check every clause after every assignment (expensive!)
    2WL approach: Only check clauses when watched literals become false

    Result: ~100x speedup on large SAT problems!

    === DATA STRUCTURE ===
    - __twl: Maps each clause to its two watched literals
    - __watch_list: Maps each literal to clauses watching it
      * Separate lists for positive and negative occurrences

    === OPERATIONS ===
    - add(clause): Add clause and assign initial watched literals
    - remove(clause): Remove clause and update watch lists
    - update_*_watched(): When watched literal becomes false, find replacement

    === EXAMPLE ===
    Clause: (P ∨ ¬Q ∨ R ∨ S)
    Initially watch: P and ¬Q
    If P becomes false: try to watch R or S instead
    If no replacement found: clause is unit or unsatisfied

    This is a sophisticated data structure - understanding it deeply requires
    studying modern SAT solver implementations!
    """
    ...
    def add(self, clause, model):
        ...
        self.__watch_list[w1][0].add(clause) if p1 else self.__watch_list[w1][1].add(
            clause
        )
        if w1 != w2:
            self.__watch_list[w2][0].add(clause) if p2 else self.__watch_list[w2][
                1
            ].add(clause)
    ...
    def remove(self, clause):
        ...
        self.__watch_list[w1][0].discard(clause) if p1 else self.__watch_list[w1][
            1
        ].discard(clause)
        if w1 != w2:
            self.__watch_list[w2][0].discard(clause) if p2 else self.__watch_list[w2][
                1
            ].discard(clause)
    ...
    def update_first_watched(self, clause, model):
        ...
        if found:  # then it will replace the watched literal
            ...
            self.__watch_list[w][0].remove(clause) if p else self.__watch_list[w][
                1
            ].remove(clause)
            self.set_second_watched(clause, new_watching)
            w, p = inspect_literal(new_watching)
            self.__watch_list[w][0].add(clause) if p else self.__watch_list[w][1].add(
                clause
            )
            return True
    ...
    def __assign_watching_literals(self, clause, model=None):
        if len(clause.args) > 2:
            if model is None or not model:
                ...
                return [
                    next(l for l in disjuncts(clause) if pl_true(l, model) is None),
...
# WALKSAT ALGORITHM
#
# WalkSAT is a randomized local search algorithm for SAT problems.
# Unlike DPLL (which is systematic), WalkSAT uses randomness and local
# improvements to find satisfying assignments.
#
# Best for: Large satisfiable formulas where finding ANY solution is enough
# Limitation: Cannot prove unsatisfiability (may run forever on UNSAT formulas)
...
def WalkSAT(clauses, p=0.5, max_flips=10000):
    ...
    === WALKSAT ALGORITHM ===
    1. Start with random truth assignment
    2. While unsatisfied clauses exist and flips < limit:
       a. Pick a random unsatisfied clause
       b. With probability p: flip random variable in clause
       c. With probability 1-p: flip variable that maximizes satisfied clauses
    3. Return model if all clauses satisfied, else None

    === INTUITION ===
    Think of it like hill-climbing with random restarts:
    - Usually make "greedy" moves (maximize satisfied clauses)
    - Sometimes make random moves to escape local optima
    - Balance between exploitation and exploration

    === PARAMETERS ===
    p: Probability of random move (vs greedy move)
       - p=0: Pure greedy (can get stuck in local optima)
       - p=1: Pure random walk (very slow)
       - p=0.5: Good balance for many problems

    max_flips: Maximum variable flips before giving up
               - Prevents infinite loops on unsatisfiable formulas
               - Larger values = more thorough search but slower

    === WHEN TO USE WALKSAT ===
    ✓ Large satisfiable formulas (1000+ variables)
    ✓ When any solution is acceptable
    ✓ When systematic methods are too slow
    ✗ When you need to prove unsatisfiability
    ✗ When you need ALL solutions

    === EXAMPLE ===
    Clauses: {P∨Q, ¬P∨R, ¬Q∨¬R}
    1. Random start: P=T, Q=T, R=T
    2. Unsatisfied: ¬Q∨¬R (both Q,R are true)
    3. Flip R to false: P=T, Q=T, R=F
    4. All clauses now satisfied → return {P:T, Q:T, R:F}
    ...
    for state in neighbors.keys():
        ...
        for n_state in adj:
            for col in colors[n_state]:
                clauses.append(
                    [expr("~" + state + "_" + col), expr("~" + n_state + "_" + col)]
                )
    ...
    def translate_to_SAT(init, transition, goal, time):
        ...
        clauses.append(
            state_sym[
                first(
                    clause[0]
                    for clause in state_sym
                    if set(conjuncts(clause[0])).issuperset(conjuncts(goal))
                ),
                time,
            ]
            ...
            for action in transition[s]:
                ...
                for t in range(time):
                    ...
                    action_sym[s, action, t] = Expr(
                        "T_{}".format(next(transition_counter))
                    )
...
#
# Unification is the key algorithm that makes First-Order Logic inference possible.
# It finds substitutions that make logical expressions identical.
#
# Example: Unify Father(x, y) with Father(John, Mary)
# Result: {x: John, y: Mary}
#
# This allows us to match general rules with specific facts!
...
def unify(x, y, s={}):
    ...
    === WHAT IS UNIFICATION? ===
    Unification finds substitutions that make two logical expressions identical.
    It's like solving equations, but with logical terms instead of numbers.

    === UNIFICATION EXAMPLES ===
    unify(Father(x), Father(John)) → {x: John}
    unify(Loves(x, x), Loves(Mary, y)) → {x: Mary, y: Mary}
    unify(P(x), Q(x)) → None (different predicates can't unify)

    === ALGORITHM STEPS ===
    1. If x and y are identical → return current substitution
    2. If x is a variable → bind x to y (with occur check)
    3. If y is a variable → bind y to x (with occur check)
    4. If both are compound expressions → unify operators and arguments
    5. If structures don't match → return None (unification fails)

    === OCCUR CHECK ===
    Prevents infinite structures like {x: f(x)}
    Example: unify(x, f(x)) fails occur check

    === SUBSTITUTION REPRESENTATION ===
    Substitutions are dictionaries: {variable: term}
    Applied left-to-right: {x: John, y: x} means x→John, then y→John

    === WHY UNIFICATION MATTERS ===
    - Enables pattern matching in logic programming
    - Core of resolution theorem proving for FOL
    - Foundation of Prolog and logic programming languages
    - Allows general rules to apply to specific situations
...
def parse_clauses_from_dimacs(dimacs_cnf):
    ...
    return map(
        ...
        map(
            lambda c: [
                expr("~X" + str(abs(l))) if l < 0 else expr("X" + str(l)) for l in c
            ],
            map(
                ...
                filter(
                    ...
                    " ".join(
                        filter(
                            ...
                            filter(
                                None, dimacs_cnf.strip().replace("\t", " ").split("\n")
                            ),
                        )
                    ).split(" 0"),
                ),
