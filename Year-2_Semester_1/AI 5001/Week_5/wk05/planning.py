...
This module implements various AI planning algorithms and problem representations.
Planning is the process of finding a sequence of actions that transforms an initial state into a desired goal state.

Key Concepts:
- STRIPS: Stanford Research Institute Problem Solver representation
- PDDL: Planning Domain Definition Language
- Fluents: Propositions that can change truth value over time
- Actions: Operations with preconditions and effects
- Delete Lists: Negative effects that remove propositions from the state

Planning Approaches Implemented:
1. State-space search (forward and backward)
2. Plan-space search (partial-order planning)
3. Constraint satisfaction planning (CSPlan)
4. Boolean satisfiability planning (SATPlan)
5. Graph-based planning (GraphPlan)
6. Hierarchical planning with High-Level Actions (HLA)
7. Angelic planning for non-deterministic domains

Example Planning Problems:
- Air Cargo: Moving cargo between airports using planes
- Blocks World: Stacking blocks in specific configurations
- Spare Tire: Changing a flat tire on a car
"""
...
class PlanningProblem:
    """
    Planning Domain Definition Language (PDDL) representation of a planning problem.

    A planning problem consists of:
    - Initial state: Set of propositions (fluents) that are true initially
    - Goal state: Set of propositions that must be true to satisfy the goal
    - Actions: Operations that can transform states
    - Domain: Optional type constraints on objects and predicates

    The state is represented as a knowledge base of first-order logic statements.
    Each state is completely defined by the conjunction of all true propositions.

    Key Terminology:
    - Fluents: Propositions that can change truth value (e.g., At(Robot, Kitchen))
    - Objects: Entities in the domain (e.g., Robot, Kitchen, Box)
    - Predicates: Relations between objects (e.g., At, On, Clear)

    Example:
        # Simple blocks world problem
        initial = 'On(A, Table) & On(B, Table) & Clear(A) & Clear(B)'
        goals = 'On(A, B)'
        actions = [Action('Move(x, y)', precond='Clear(x)', effect='On(x, y) & ~Clear(y)')]
    """
    ...
    def __init__(self, initial, goals, actions, domain=None):
        self.initial = (
            ...
            if domain is None
            else self.convert(initial) + self.convert(domain)
        )
    ...
    def convert(self, clauses):
        """
        Converts string representations into Expr objects for logical processing.

        This method handles the conversion of planning problem statements from
        human-readable strings to internal logical expressions that can be
        manipulated by the planning algorithms.

        Args:
            clauses: String, Expr, or list of logical statements

        Returns:
            List of Expr objects representing the converted clauses

        Example:
            convert("At(Robot, Kitchen) & On(Box, Table)")
            # Returns: [At(Robot, Kitchen), On(Box, Table)]
        """
    ...
    def expand_fluents(self, name=None):
        """
        Generate all possible ground fluents (propositions) from the problem.

        Fluents are propositions that can change truth value during planning.
        This method takes abstract fluent schemas and grounds them with all
        possible combinations of objects from the domain.

        For example, if we have objects {A, B} and fluent On(x, y), this generates:
        [On(A, A), On(A, B), On(B, A), On(B, B)]

        Args:
            name: Optional specific fluent name to expand (default: expand all)

        Returns:
            List of all possible ground fluents in the domain

        Note: This is used for CSP-based planning where we need to enumerate
        all possible propositions that could be true or false at any time step.
        """
            ...
            for action in self.actions:
                if action.precond:
                    for fests in (
                        set(action.precond)
                        .union(action.effect)
                        .difference(self.convert(action.domain))
                    ):
                        if fests.op[:3] != "Not":
        ...
        if name is not None:
            ...
            fluent_list = list(
                map(
                    lambda fluent: Expr(fluent[0], *fluent[1]),
                    {
                        fluent.op: fluent.args
                        for fluent in self.initial
                        + self.goals
                        + [
                            clause
                            for action in self.actions
    ...
    def expand_actions(self, name=None):
        ...
        for action in action_list:
            for permutation in itertools.permutations(objects, len(action.args)):
                bindings = unify_mm(
                    Expr(action.name, *action.args), Expr(action.name, *permutation)
                )
                if bindings is not None:
                    ...
                    if (
                        (has_domains and kb.ask(new_expr) is not False)
                        or (has_domains and not action.precond)
                        or not has_domains
                    ):
                        new_preconds = []
    ...
    def is_strips(self):
        """
        Check if this problem follows STRIPS representation.

        STRIPS (Stanford Research Institute Problem Solver) is a restricted form of planning where:
        - Preconditions and goals contain only positive literals (no negation)
        - Effects can have both positive and negative literals

        This restriction makes planning more efficient by avoiding the complexity of reasoning with negative preconditions.

        Returns:
            bool: True if problem has no negative literals in preconditions/goals

        Example:
            # STRIPS-compliant: precond='Clear(x)', goal='On(A,B)'
            # Non-STRIPS: precond='~Broken(x)', goal='~Empty(room)'
        """
        return all(clause.op[:3] != "Not" for clause in self.goals) and all(
            clause.op[:3] != "Not"
            for action in self.actions
            for clause in action.precond
        )
...
class Action:
    """
    Represents an action schema in planning problems.

    An action defines how the world can be changed, consisting of:
    - Name and parameters: What the action is called and what objects it operates on
    - Preconditions: What must be true before the action can be executed
    - Effects: What becomes true/false after executing the action
    - Domain constraints: Optional type restrictions on parameters

    Actions use variables (e.g., 'x', 'y') that get bound to specific objects
    when the action is instantiated (grounded) during planning.

    Negative effects (delete list) are represented with 'Not' prefix.

    Example:
        # Move action in blocks world
        action = Action('Move(block, from, to)',
                       precond=['On(block, from)', 'Clear(block)', 'Clear(to)'],
                       effect=['On(block, to)', 'Clear(from)',
                              '~On(block, from)', '~Clear(to)'])

    The action Move(A, Table, B) would:
    - Require: A is on Table, A is clear, B is clear
    - Result in: A is on B, Table becomes clear, A no longer on Table, B no longer clear
    """
    ...
    def __init__(self, action, precond, effect, domain=None):
        ...
        self.precond = (
            self.convert(precond)
            if domain is None
            else self.convert(precond) + self.convert(domain)
        )
    ...
    def relaxed(self):
        """
        Create a relaxed version of this action by removing the delete list.

        A relaxed action only has positive effects (add list) and ignores
        negative effects (delete list). This is used in planning heuristics
        because relaxed problems are easier to solve and provide optimistic
        estimates of the real solution cost.

        The relaxed planning problem always has a solution if the original
        problem has one, and the relaxed solution length is a lower bound
        on the optimal solution length.

        Returns:
            Action: New action with only positive effects

        Example:
            Original: Move(A,B,C) effects=['On(A,C)', '~On(A,B)', 'Clear(B)']
            Relaxed:  Move(A,B,C) effects=['On(A,C)', 'Clear(B)']
        """
        return Action(
            Expr(self.name, *self.args),
            self.precond,
    ...
    def substitute(self, e, args):
        """
        Replace variables in expression with concrete objects (variable grounding).

        This method transforms an abstract action schema into a concrete action
        by substituting variables with specific objects. This process is called
        "grounding" the action.

        Args:
            e: Expression containing variables to be substituted
            args: List of concrete objects to substitute for variables

        Returns:
            Expr: New expression with variables replaced by concrete objects

        Example:
            # Action schema: Move(x, y, z)
            # Variables: [x, y, z]
            # Concrete args: [BlockA, Table, BlockB]
            # Expression: On(x, y) becomes On(BlockA, Table)
        """
...
def air_cargo():
    ...
    A logistics planning problem involving transportation of cargo between cities using planes.
    This is a classic example of how planning applies to real-world logistics and scheduling.

    Problem Setup:
    - Two cities: SFO (San Francisco), JFK (New York)
    - Two planes: P1 (starts at SFO), P2 (starts at JFK)
    - Two cargo items: C1 (starts at SFO), C2 (starts at JFK)

    Initial State:
    - At(C1, SFO), At(C2, JFK)  # cargo locations
    - At(P1, SFO), At(P2, JFK)  # plane locations

    Goal: Get both cargo items to their opposite cities
    - At(C1, JFK) and At(C2, SFO)

    Available Actions:
    1. Load(cargo, plane, airport) - load cargo onto plane
    2. Unload(cargo, plane, airport) - unload cargo from plane
    3. Fly(plane, from_airport, to_airport) - fly plane between cities
...
def three_block_tower():
    """
    THREE-BLOCK-TOWER (The Sussman Anomaly)

    A classic blocks-world problem that demonstrates the need for non-linear planning.
    The goal is to stack three blocks A, B, C such that A is on B and B is on C.

    Initial State: A and B are on table, C is on A, B and C are clear
        [C]
        [A] [B]
      ----------------
           Table

    Goal State: A on B, B on C (tower configuration)
        [A]
        [B]
        [C]
      --------
    ...
    Why it's called "Sussman Anomaly":
    - Linear planners fail because they try to achieve goals in order
    - Goal 1: On(A,B) - can't achieve directly because C is on A
    - Goal 2: On(B,C) - can't achieve directly because B needs to be clear
    - Solution requires interleaving: clear A first, then move B to C, then A to B

    This problem demonstrates that optimal planning requires considering
    goal interactions and cannot always proceed by achieving goals independently.
    ...
    >>> tbt.goal_test()
    False
    >>> tbt.act(expr('MoveToTable(C, A)'))  # First clear block A
    >>> tbt.act(expr('Move(B, Table, C)'))  # Then put B on C
    >>> tbt.goal_test()
    False
    >>> tbt.act(expr('Move(A, Table, B)'))  # Finally put A on B
    >>> tbt.goal_test()
    True
    >>>
    """
    return PlanningProblem(
        initial="On(A, Table) & On(B, Table) & On(C, A) & Clear(B) & Clear(C)",
        goals="On(A, B) & On(B, C)",
        actions=[
            Action(
                "Move(b, x, y)",
...
def have_cake_and_eat_cake_too():
    ...
    return PlanningProblem(
        ...
        actions=[
            Action(
                "Eat(Cake)", precond="Have(Cake)", effect="Eaten(Cake) & ~Have(Cake)"
            ),
...
def shopping_problem():
    ...
    return PlanningProblem(
        initial="At(Home) & Sells(SM, Milk) & Sells(SM, Banana) & Sells(HW, Drill)",
...
class ForwardPlan(search.Problem):
    """
    [Section 10.2.1] Forward State-Space Search for Planning

    Forward planning searches from the initial state toward the goal by applying
    actions and generating successor states. This is similar to standard search
    problems, but operates in the space of world states rather than locations.

    Algorithm:
    1. Start with initial state
    2. Generate all applicable actions (those whose preconditions are satisfied)
    3. Apply each action to get successor states
    4. Continue until goal state is reached

    Advantages:
    - Intuitive: follows natural progression from start to goal
    - Only considers reachable states

    Disadvantages:
    - Can explore many irrelevant states
    - Goal may be hard to reach from initial state

    The search uses heuristics based on relaxed planning problems to guide
    the search toward promising states.
    """

    def __init__(self, planning_problem):
        super().__init__(
            associate("&", planning_problem.initial),
            associate("&", planning_problem.goals),
        )
    ...
    def actions(self, state):
        return [
            ...
            if all(pre in conjuncts(state) for pre in action.precond)
        ]
    ...
    def h(self, state):
        """
        Compute ignore-delete-lists heuristic for forward search.

        This heuristic creates a relaxed version of the planning problem by
        removing all negative effects (delete lists) from actions. The relaxed
        problem is easier to solve and provides an optimistic estimate of
        the remaining cost to reach the goal.

        Relaxation Process:
        1. Remove all negative effects from actions (only keep positive effects)
        2. Solve the relaxed problem using GraphPlan
        3. Use the solution length as the heuristic value

        Properties:
        - Admissible: Never overestimates the true cost (good for A* search)
        - Informative: Usually much better than zero heuristic
        - Fast to compute: Relaxed problems are easier to solve

        The key insight is that if you can't solve the relaxed problem,
        you definitely can't solve the original problem. If you can solve
        the relaxed problem in n steps, the original problem needs at least n steps.
        """
        # Create relaxed planning problem (remove negative effects from actions)
        ...
        try:
            # Solve relaxed problem and return solution length as heuristic
            return len(linearize(GraphPlan(relaxed_planning_problem).execute()))
        except:
            # If relaxed problem unsolvable, return infinite cost
            return np.inf


class BackwardPlan(search.Problem):
    """
    [Section 10.2.2] Backward Relevant-States Search for Planning

    Backward planning searches from the goal toward the initial state by finding
    actions that could achieve the current subgoals. This approach can be more
    efficient when there are fewer ways to achieve goals than to progress
    from the initial state.

    Algorithm:
    1. Start with goal conditions as subgoals
    2. Find actions that can achieve some subgoal (relevant actions)
    3. Replace achieved subgoals with action preconditions
    4. Continue until initial state satisfies all subgoals

    Action Relevance Criteria:
    - Action achieves at least one subgoal in its effects
    - Action doesn't delete any required subgoal
    - Action preconditions are consistent with remaining subgoals

    Advantages:
    - Focuses on goal-relevant actions only
    ...
    def actions(self, subgoal):
        ...
        def negate_clause(clause):
            return (
                ...
                if clause.op[:3] == "Not"
                else Expr("Not" + clause.op, *clause.args)
            )
        ...
        return [
            ...
            for action in self.expanded_actions
            if (
                any(prop in action.effect for prop in subgoal)
                and not any(negate_clause(prop) in subgoal for prop in action.effect)
                and not any(
    ...
    def result(self, subgoal, action):
        ...
        return associate(
            "&",
            set(set(conjuncts(subgoal)).difference(action.effect)).union(
                action.precond
            ),
        )
...
def CSPlan(
    planning_problem, solution_length, CSP_solver=ac_search_solver, arc_heuristic=sat_up
):
    """
    [Section 10.4.3] Planning as Constraint Satisfaction Problem

    CSPlan converts a planning problem into a Constraint Satisfaction Problem (CSP)
    by encoding the planning problem across time steps (planning horizon).

    Key Idea: If we know the solution length, we can encode the entire planning
    process as constraints over variables representing fluents and actions at
    each time step.

    Encoding:
    - Variables: action_t (action at time t), fluent_t (fluent truth at time t)
    - Domains: actions ∈ {possible actions}, fluents ∈ {True, False}
    - Constraints:
        * Initial state: fluent_0 values fixed
        * Goal state: fluent_T values must satisfy goals
        * Precondition: if action_t = A, then preconditions of A true at t
        * Effect: if action_t = A, then effects of A true at t+1
        * Frame: fluents persist unless changed by actions

    Args:
        planning_problem: The planning problem to solve
        solution_length: Maximum number of time steps (planning horizon)
        CSP_solver: Algorithm to solve the resulting CSP
        arc_heuristic: Heuristic for constraint propagation

    Returns:
        List of actions forming a solution, or None if no solution exists
    ...
    def st(var, stage):
        """Create variable name for CSP encoding: variable at time stage"""
        return str(var) + "_" + str(stage)

    def if_(v1, v2):
        """Create implication constraint: if x2 == v2 then x1 == v1"""
    ...
    def eq_if_not_in_(actset):
        """Create frame constraint: variables stay equal unless action in actset changes them"""

        def eq_if_not_in(x1, a, x2):
            return x1 == x2 if a not in actset else True

        eq_if_not_in.__name__ = (
            "first and third arguments are equal if action is not in "
            + str(actset)
            + " "
        )
        return eq_if_not_in

    # Generate all possible ground actions and fluents for CSP variables
    expanded_actions = planning_problem.expand_actions()
    fluent_values = planning_problem.expand_fluents()

    # Try different planning horizons until solution found
    for horizon in range(solution_length):
        # Create CSP variables for each time step
        act_vars = [st("action", stage) for stage in range(horizon + 1)]
        domains = {
            av: list(map(lambda action: expr(str(action)), expanded_actions))
            for av in act_vars
        }
        domains.update(
            {
                st(var, stage): {True, False}
                for var in fluent_values
                for stage in range(horizon + 2)
            }
        )

        # CONSTRAINT TYPE 1: Initial state constraints
        # Fix the truth values of fluents at time 0 to match initial state
        constraints = [
            Constraint((st(var, 0),), is_constraint(val))
            for (var, val) in {
                expr(str(fluent).replace("Not", "")): True
                if fluent.op[:3] != "Not"
                else False
        ...
        constraints += [
            Constraint((st(var, 0),), is_constraint(False))
            for var in {
                expr(str(fluent).replace("Not", ""))
                for fluent in fluent_values
                if fluent not in planning_problem.initial
            }
        ]

        # CONSTRAINT TYPE 2: Goal state constraints
        # Require goal fluents to be true at final time step
        constraints += [
        ...
        # CONSTRAINT TYPE 3: Precondition constraints
        # If action A is executed at time t, its preconditions must be true at time t
        constraints += [
        ...
        # CONSTRAINT TYPE 4: Effect constraints
        # If action A is executed at time t, its effects must be true at time t+1
        constraints += [
            Constraint((st(var, stage + 1), st("action", stage)), if_(val, act))
        ...
        # CONSTRAINT TYPE 5: Frame constraints (persistence)
        # Fluents remain unchanged unless affected by an action
        constraints += [
            Constraint(
...
def SATPlan(planning_problem, solution_length, SAT_solver=cdcl_satisfiable):
    ...
    def expand_transitions(state, actions):
        ...
        for action in filter(lambda act: act.check_precond(state, act.args), actions):
            transition[associate("&", state)].update(
                {
                    Expr(action.name, *action.args): associate(
                        "&",
                        sorted(
                            set(
                                filter(
                                    lambda clause: clause.op[:3] != "Not",
    ...
    expand_transitions(
        associate("&", planning_problem.initial), planning_problem.expand_actions()
    )

    return SAT_plan(
        associate("&", sorted(planning_problem.initial)),
        transition,
        associate("&", sorted(planning_problem.goals)),
        solution_length,
...
class Level:
    """
    Represents a single level in the GraphPlan planning graph.

    A level contains either propositions (state level) or actions (action level).
    The planning graph alternates between these two types of levels:

    State Level (Proposition Level):
    - Contains propositions (fluents) that can be true at this time step
    - Tracks which actions can add/achieve each proposition
    - Tracks mutual exclusions between propositions

    Action Level:
    - Contains actions that can be executed at this time step
    - Actions are applicable if their preconditions are satisfied
    - Tracks mutual exclusions between actions

    Mutex Relations:
    - Inconsistent Effects: Actions with contradictory effects
    - Interference: Action deletes precondition of another action
    - Competing Needs: Actions require mutually exclusive preconditions
    - Inconsistent Support: Propositions supported by mutex actions

    The Level class manages the complex relationships between actions and
    propositions needed for GraphPlan's mutex analysis.
    """
    ...
    def find_mutex(self):
        """
        Identify mutually exclusive (mutex) actions and propositions.

        Mutex relations are critical for GraphPlan's correctness and efficiency.
        Two actions are mutex if they cannot be executed simultaneously.
        Two propositions are mutex if they cannot both be true simultaneously.

        Types of Action Mutex:
        1. Inconsistent Effects: One action adds P, another adds ~P
        2. Interference: One action deletes precondition of another
        3. Competing Needs: Actions require mutex preconditions

        Types of Proposition Mutex:
        1. Inconsistent Support: All ways to achieve propositions are mutex

        The mutex information is used during solution extraction to ensure
        only valid (non-conflicting) action combinations are considered.
        """
...
class GraphPlan:
    """
    Implementation of the GraphPlan Algorithm

    GraphPlan constructs a planning graph that alternates between proposition
    levels (states) and action levels. The graph is built incrementally until
    all goals appear in a proposition level without mutual exclusions.

    Algorithm Overview:
    1. Build planning graph level by level:
       - Level 0: Initial state propositions
       - Level 1: Actions applicable from level 0
       - Level 2: Propositions resulting from level 1 actions
       - Continue until goals appear non-mutex or graph levels off

    2. Extract solution by backward search through graph:
       - Start from goal propositions at final level
       - Find non-mutex actions that achieve goals
       - Recursively find actions for action preconditions
       - Continue until initial state is reached

    Key Concepts:
    - Mutex (Mutual Exclusion): Two actions/propositions that cannot
      occur simultaneously due to conflicts
    - Level-off: When two consecutive levels have identical propositions
    - No-goods: Failed goal sets that cannot be achieved
    ...
    def check_leveloff(self):
        ...
        check = set(self.graph.levels[-1].current_state) == set(
            self.graph.levels[-2].current_state
        )
...
class PartialOrderPlanner:
    """
    [Section 10.13] PARTIAL-ORDER-PLANNER (POP Algorithm)

    Partial-order planning constructs plans by searching in the space of plans
    rather than the space of states. It builds plans incrementally by adding
    actions and ordering constraints as needed.

    Key Concepts:
    - Partial Order: Some actions must occur before others (A < B)
    - Causal Links: Action A achieves condition C for action B (A --C--> B)
    - Open Preconditions: Subgoals that still need to be achieved
    - Threats: Actions that might interfere with established causal links

    Algorithm Steps:
    1. Select an open precondition (subgoal to achieve)
    2. Choose an action that can achieve this precondition
    3. Add ordering constraints to maintain consistency
    4. Protect existing causal links from threats
    5. Update the agenda of open preconditions
    6. Add new causal links
    7. Resolve any threats by promotion or demotion
    8. Repeat until no open preconditions remain

    Threat Resolution:
    - Promotion: Force threatening action before causal link
    - Demotion: Force threatening action after causal link

    Advantages:
    - Finds partially ordered plans (more flexible than total order)
    - Commitment only when necessary (least commitment principle)
    - Can handle complex action interactions

    Disadvantages:
    - Complex bookkeeping of constraints and causal links
    - Can be slower than state-space search for simple problems

    The planner maintains:
    - Actions: Set of actions in the plan
    - Constraints: Ordering constraints between actions
    - Causal Links: Dependencies between actions
    - Agenda: Open preconditions that need to be achieved
    """
    ...
    def is_a_threat(self, precondition, effect):
        ...
        if (str(effect.op) == "Not" + str(precondition.op)) or (
            "Not" + str(effect.op) == str(precondition.op)
        ):
        ...
        if action != causal_link[0] and action != causal_link[2] and threat:
            ...
            if not self.cyclic(new_constraints):
                ...
                if not self.cyclic(new_constraints):
                    ...
                    print(
                        "Unable to resolve a threat caused by",
                        action,
                        "onto",
                        causal_link,
                    )
    ...
    def toposort(self, graph):
        ...
        extra_elements_in_dependencies = _reduce(set.union, graph.values()) - set(
            graph.keys()
        )
        ...
        while True:
            ordered = set(
                element for element, dependency in graph.items() if len(dependency) == 0
            )
            if not ordered:
                break
            yield ordered
            graph = {
                element: (dependency - ordered)
    ...
    def execute(self, display=True):
        ...
        while len(self.agenda) > 0:
            ...
            for action in self.actions:
                self.constraints = self.protect(
                    (act0, G, act1), action, self.constraints
                )
...
class HLA(Action):
    """
    High-Level Action (HLA) for real-world planning with resource constraints.

    HLAs extend basic actions to handle practical planning considerations:
    - Resource Management: Actions consume or use limited resources
    - Temporal Constraints: Actions have execution durations
    - Job Ordering: Some actions must complete before others can start

    Resource Types:
    - Consumable Resources: Used up when action executes (e.g., fuel, materials)
    - Reusable Resources: Temporarily unavailable during action (e.g., tools, machines)

    Real-World Constraints:
    - Duration: How long the action takes to execute
    - Resource Requirements: What resources are needed
    - Precedence Constraints: Ordering dependencies between actions

    Example - Car Assembly:
        add_engine = HLA('AddEngine(car)',
                        duration=30,           # 30 minutes
                        use={'Crane': 1},      # Need 1 crane (reusable)
                        consume={'Bolts': 4})  # Use 4 bolts (consumable)

    HLAs can be hierarchically decomposed into more primitive actions,
    enabling planning at multiple levels of abstraction.

    Advantages:
    - Models real-world resource limitations
    - Supports hierarchical planning
    - Handles temporal and scheduling constraints
    ...
    def __init__(
        self, action, precond=None, effect=None, duration=0, consume=None, use=None
    ):
        """
    ...
    def do_action(self, job_order, available_resources, kb, args):
        ...
        if not self.has_usable_resource(available_resources):
            raise Exception(
                "Not enough usable resources to execute {}".format(self.name)
            )
        if not self.has_consumable_resource(available_resources):
            raise Exception(
                "Not enough consumable resources to execute {}".format(self.name)
            )
        if not self.inorder(job_order):
            raise Exception(
                "Can't execute {} - execute prerequisite actions first".format(
                    self.name
                )
            )
...
class RealWorldPlanningProblem(PlanningProblem):
    ...
    def act(self, action):
        ...
        self.initial = list_action.do_action(
            self.jobs, self.resources, self.initial, args
        ).clauses
    ...
    def refinements(self, library):  # refinements may be (multiple) HLA themselves ...
        ...
        for i in indices:
            ...
            for j in range(len(library["steps"][i])):
                ...
                index_step = [
                    k
                    for k, x in enumerate(library["HLA"])
                    if x == library["steps"][i][j]
                ][0]
    ...
    def hierarchical_search(self, hierarchy):
        ...
        while True:
            ...
            outcome = RealWorldPlanningProblem(
                RealWorldPlanningProblem.result(self.initial, prefix),
                self.goals,
                self.actions,
            )
            ...
            if not hla:  # hla is None and plan is primitive
                ...
                for sequence in RealWorldPlanningProblem.refinements(
                    hla, hierarchy
                ):  # find refinements
                    frontier.append(
                        Node(outcome.initial, plan, prefix + sequence + suffix)
                    )
    ...
    def angelic_search(self, hierarchy, initial_plan):
        ...
        while True:
            ...
            if self.intersects_goal(opt_reachable_set):
                ...
                if guaranteed and RealWorldPlanningProblem.making_progress(
                    plan, initial_plan
                ):
                    final_state = guaranteed[0]  # any element of guaranteed
                    return RealWorldPlanningProblem.decompose(
                        hierarchy, final_state, pes_reachable_set
                    )
    ...
    def intersects_goal(self, reachable_set):
        ...
        return [
            y
            for x in list(reachable_set.keys())
    ...
    def reach_opt(init, plan):
        ...
        optimistic_description = (
            plan.action
        )  # list of angelic actions with optimistic description
        return RealWorldPlanningProblem.find_reachable_set(
            reachable_set, optimistic_description
        )
    ...
    def reach_pes(init, plan):
        ...
        """
        reachable_set = {0: [init]}
        pessimistic_description = (
            plan.action_pes
        )  # list of angelic actions with pessimistic description
        return RealWorldPlanningProblem.find_reachable_set(
            reachable_set, pessimistic_description
        )
    ...
    def find_hla(plan, hierarchy):
        ...
        for i in range(
            len(plan.action)
        ):  # find the first HLA in plan, that is not primitive
    ...
    def find_previous_state(s_f, reachable_set, i, action):
        ...
        for state in reachable_set[i - 1]:
            if s_f in [
                x
                for x in RealWorldPlanningProblem.reach_pes(
...
def go_to_sfo():
    ...
    go_home_sfo1 = HLA(
        "Go(Home, SFO)", precond="At(Home) & Have(Car)", effect="At(SFO) & ~At(Home)"
    )
    go_home_sfo2 = HLA(
        "Go(Home, SFO)", precond="At(Home)", effect="At(SFO) & ~At(Home)"
    )
    drive_home_sfoltp = HLA(
        "Drive(Home, SFOLongTermParking)",
        precond="At(Home) & Have(Car)",
        effect="At(SFOLongTermParking) & ~At(Home)",
    )
    shuttle_sfoltp_sfo = HLA(
        "Shuttle(SFOLongTermParking, SFO)",
        precond="At(SFOLongTermParking)",
        effect="At(SFO) & ~At(SFOLongTermParking)",
    )
    taxi_home_sfo = HLA(
        "Taxi(Home, SFO)", precond="At(Home)", effect="At(SFO) & ~At(Home)"
    )
    ...
    return RealWorldPlanningProblem(
        initial="At(Home)", goals="At(SFO)", actions=actions
    ), library


class AngelicHLA(HLA):
    """
    High-Level Action with Angelic Semantics for Non-Deterministic Planning

    Angelic semantics allow actions to have uncertain or non-deterministic effects.
    Instead of knowing exactly what will happen, we know what might happen.
    This is useful for planning under uncertainty or with incomplete information.

    Angelic Effect Operators:
    - ~P  : Definitely remove proposition P
    - P   : Definitely add proposition P
    - $+P : Possibly add proposition P (might or might not happen)
    - $-P : Possibly remove proposition P (might or might not happen)
    - $$P : Possibly add or remove P (could go either way)

    Optimistic vs Pessimistic Reasoning:
    - Optimistic: Assume best-case outcomes when planning
    - Pessimistic: Assume worst-case outcomes when checking feasibility

    Example:
        # Driving action with uncertain outcome
        drive = AngelicHLA('Drive(Home, Work)',
                          effect='$+At(Work) & $-At(Home)')
        # Might successfully reach work, might not

    The planner can reason about:
    - What goals are possibly achievable (optimistic reachability)
    - What goals are guaranteed achievable (pessimistic reachability)

    This enables planning in domains where:
    - Actions have uncertain outcomes
    - Information is incomplete
    - Multiple contingencies must be considered

    Applications:
    - Robot navigation with uncertainty
    - Medical treatment planning
    - Business process planning with external dependencies
    """
    ...
    def convert(self, clauses):
        ...
        """
        lib = {"~": "Not", "$+": "PosYes", "$-": "PosNot", "$$": "PosYesNot"}
    ...
    def angelic_action(self):
        """
        Convert an angelic HLA into all possible concrete HLAs it represents.

        An angelic action with uncertain effects represents multiple possible
        concrete actions. This method enumerates all possibilities by considering
        each combination of optimistic and pessimistic outcomes.

        Expansion Rules:
        - Definite effects (P, ~P): Always included
        - Possibly add ($+P): Two possibilities - add P or leave unchanged
        - Possibly remove ($-P): Two possibilities - remove P or leave unchanged
        - Possibly add/remove ($$P): Three possibilities - add P, remove P, or leave unchanged

        Example Expansion:
        Angelic action: Go(Home,Work) with effect '$+At(Work) & $$Have(Car)'

        Expands to 6 concrete actions:
        1. At(Work) & Have(Car)      # Successfully drive, keep car
        2. At(Work) & ~Have(Car)     # Successfully drive, lose car
        3. At(Work)                  # Successfully drive, car unchanged
        4. Have(Car)                 # Fail to drive, keep car
        5. ~Have(Car)                # Fail to drive, lose car
        6. (no effect)               # Fail to drive, car unchanged

        This expansion allows the planner to reason about all possible outcomes
        and choose actions that work under different contingencies.

        Returns:
            List[HLA]: All concrete HLAs this angelic action could represent
        """
                ...
                if effects[i]:
                    if clause.args:
                        effects[i] = expr(
                            str(effects[i])
                            + "&"
                            + str(Expr(clause.op[w:], clause.args[0]))
                        )  # make changes in the ith part of effects
                        if n == 3:
                            effects[i + len(effects) // 3] = expr(
                                str(effects[i + len(effects) // 3])
                                + "&"
                                + str(Expr(clause.op[6:], clause.args[0]))
                            )
                    else:
                        effects[i] = expr(
                        ...
                        if n == 3:
                            effects[i + len(effects) // 3] = expr(
                                str(effects[i + len(effects) // 3])
                                + "&"
                                + str(expr(clause.op[6:]))
                            )

                else:
                    if clause.args:
                        effects[i] = Expr(
                            clause.op[w:], clause.args[0]
                        )  # make changes in the ith part of effects
                        if n == 3:
                            effects[i + len(effects) // 3] = Expr(
                                clause.op[6:], clause.args[0]
                            )
        ...
        return [
            HLA(Expr(self.name, self.args), self.precond, effects[i])
    ...
    def compute_parameters(clause):
        ...
        if clause.op[:9] == "PosYesNot":
            ...
            clause.op[:6] == "PosYes"
        ):  # possibly add variable: two possible effects for the variable
            n = 2
            w = 6
        elif (
            clause.op[:6] == "PosNot"
        ):  # possibly remove variable: two possible effects for the variable
            n = 2
...
class AngelicNode(Node):
    ...
    def __init__(
        self, state, parent=None, action_opt=None, action_pes=None, path_cost=0
    ):
